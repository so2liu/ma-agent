...
function AppPanel() {
  ...
  return (
    <div className="app-panel">
      ...
      {apps.length === 0 && (
        <p className="text-muted">在对话中告诉 Claude 你想要什么应用， 它会帮你生成到这里。</p>
      )}
    </div>
  );
}
    ...
    <div className="app-card">
      ...
      : <div className="app-running">
          <div className="lan-url">
            ...
            <button onClick={() => copyToClipboard(app.lanUrl)}>复制链接</button>
          </div>
...
function createDBApi(dataPath: string) {
  ...
  return {
    ...
    query: (filter: Record<string, unknown>) => {
      return load().filter((record) => Object.entries(filter).every(([k, v]) => record[k] === v));
    }
  };
}
```
